...
class bananaWavelets:
    def __init__(self):
        self.x = self.y = self.f = self.a = self.c = self.s = self.sigmaX = self.sigmaY = 0
        
def bananaParameters():
    bananaWavelets.x = 30 #rows
    bananaWavelets.y = 30 #columns
    bananaWavelets.f = 0.28 #frequency
    bananaWavelets.a = 16*(pi/8) #orientation
    bananaWavelets.c = 0.06 #curvature
    bananaWavelets.s = 1.0 #size
    bananaWavelets.sigmaX = 0.75 #scale of Gaussian filter in x direction
    bananaWavelets.sigmaY = 3 #scale of Gaussian filter in y direction

def banana_wavelet(x, y, f, a, c, s, sigma_x, sigma_y, u, v):
